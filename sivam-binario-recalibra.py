...
while(script_live):
    ...
    quatRecord_raw = []  # lista Python pura — sem overhead OpenSim no loop
    #
    ...
    while(running):
        if (not b.empty()) or (t == sim_steps): # new button press so we should save the data and restart the sim
            ...
            if log_data:
                ...
                # converte lista → TimeSeriesTableQuaternion só na hora de salvar
                quatRecord = osim.TimeSeriesTableQuaternion()
                set_column_labels(quatRecord, labels)
                for ts, qi in quatRecord_raw:
                    append_quat_row(quatRecord, ts, qi)
        ...
        quatRecord_raw.append((time_s, Qi))  #save quaternions (lista nativa, sem C++)
